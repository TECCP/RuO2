program mk_band_kanamori_af
! ifort -O -o mk_band_kanamori_af mk_band_kanamori_af.f90 -L/opt/intel/mkl/10.2.5.035/lib/em64t -lmkl_lapack95_lp64 -lmkl_intel_lp64 -lmkl_core -lmkl_sequential -lpthread -lm

implicit none

integer*4  i,j,k,l,nr,ii,jj,kpt,nh,bh,natom,ia
integer*4  i1,i2,kzero,count,count2
parameter (nr=2057,kpt=999,nh=12,natom=2,bh=nh/natom)
integer*4  avec(3,nr)
...
! for band
real*8     xval
parameter (xval=3.81099d0)
complex*16 bandup(bh,bh),banddn(bh,bh),hk2(nh,nh,kpt)
integer*4  new(nh)

! for zheev
integer*4  lwork,info
parameter (lwork=300)
real*8     ene(nh),rwork(300)
complex*16 work(lwork)

open( 99,file='wannier90_hr.dat',status='old',    form='formatted')
open(100,file='klist_band.in',   status='old',    form='formatted')
open( 81,file='n.dmft',          status='old',    form='formatted')
open( 82,file='u.in',            status='old',    form='formatted')
open( 93,file='band_hmf_up.out', status='unknown',form='formatted')
open( 94,file='band_hmf_dn.out', status='unknown',form='formatted')
...
do l=1,kpt
   ...
   write(93,9992)dble(l-1)/dble(kpt-1)*3.81099d0,(ene(i),i=1,bh)
   do i=1,bh
      do j=1,bh
         banddn(i,j)=hk(i+bh,j+bh,l)
      enddo
   enddo
   call zheev('V','U',bh,banddn,bh,ene,work,lwork,rwork,info)
   write(94,9992)dble(l-1)/dble(kpt-1)*3.81099d0,(ene(i),i=1,bh)
enddo
!-----[end]
...
stop
end program mk_band_kanamori_af
